...
use crate::schema::api::mail_accounts::{Create, List, Patch};
use crate::schema::model::{MailAccount, Paginated};
...
pub trait MailAccounts {
    ...
    async fn test(&self, body: &Create) -> Result<Response<Item>>;
    ...
    async fn process(&self, id: i32) -> Result<Response<()>>;
...
impl MailAccounts for Client {
    ...
    async fn patch(&self, item: &mut Item, body: &Patch) -> Result<Response<()>> {
        let path = format!("/api/mail_accounts/{}/", item.id);
    ...
    async fn process(&self, id: i32) -> Result<Response<()>> {
        ...
        let req = self.build_request(Method::POST, &path, params::NONE, body::none)?;
        let resp = self.send_request(req).await?;
        Self::ignore_content(resp)
    }
